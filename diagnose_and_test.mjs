...
function runDiagnostics() {
    ...
    for (const acc of accounts) {
        console.log(`Scheduling test tweet for account: ${acc.name} (ID: ${acc.id})`);
        const stmt = db.prepare(`
            INSERT INTO tweets (content, status, scheduled_at, account_id)
            VALUES (?, ?, ?, ?)
        `);
        stmt.run(`Multi-account test from TweetFlow for @${acc.name} at ${new Date().toLocaleString()}`, 'pending', now - 1000, acc.id);
    }
