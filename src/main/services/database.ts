...
export class DatabaseService {
  ...
  init() {
    ...
    const hasErrorMessage = tableInfo.some(
      (col: any) => col.name === "error_message",
    );
    if (!hasErrorMessage) {
    ...
    if (!hasMode) {
      this.db.exec(
        "ALTER TABLE tweets ADD COLUMN mode TEXT DEFAULT 'scheduled'",
      );
    }
    const hasRecurring = tableInfo.some(
      (col: any) => col.name === "is_recurring",
    );
    if (!hasRecurring) {
      this.db.exec(
        "ALTER TABLE tweets ADD COLUMN is_recurring INTEGER DEFAULT 0",
      );
    }
    const hasSendCount = tableInfo.some(
      (col: any) => col.name === "send_count",
    );
    if (!hasSendCount) {
      this.db.exec(
        "ALTER TABLE tweets ADD COLUMN send_count INTEGER DEFAULT 0",
      );
    }
    const hasRecurrenceInterval = tableInfo.some(
      (col: any) => col.name === "recurrence_interval",
    );
    if (!hasRecurrenceInterval) {
      this.db.exec(
        "ALTER TABLE tweets ADD COLUMN recurrence_interval INTEGER DEFAULT 15552000000",
      );
    }
    const hasIsDeletedTweet = tableInfo.some(
    ...
    const hasIsDeleted = tableInfoLogs.some(
      (col: any) => col.name === "is_deleted",
    );
    if (!hasIsDeleted) {
      this.db.exec(
    ...
    // Seed default slots
    const slotCount = this.db
      .prepare("SELECT COUNT(*) as count FROM queue_slots")
      .get() as { count: number };
    if (slotCount.count === 0) {
      const insertSlot = this.db.prepare(
        "INSERT INTO queue_slots (time_str) VALUES (?)",
      );
    ...
    const logCount = this.db
      .prepare("SELECT COUNT(*) as count FROM send_logs")
      .get() as { count: number };
    if (logCount.count === 0) {
      const legacySentTweets = this.db
        .prepare("SELECT * FROM tweets WHERE status = 'sent'")
        .all() as any[];
  ...
  updateTweetStatus(
    ...
    const stmt = this.db.prepare(
      "UPDATE tweets SET status = ?, tweet_id = ?, error_message = ? WHERE id = ?",
    );
  ...
  getDrafts(): TweetRecord[] {
    const stmt = this.db.prepare(
      "SELECT * FROM tweets WHERE status = 'draft' AND is_deleted = 0 ORDER BY id DESC",
    );
  ...
  getQueueSlots(): QueueSlot[] {
    return this.db
      .prepare("SELECT * FROM queue_slots ORDER BY time_str ASC")
      .all() as QueueSlot[];
  }
  ...
  updateQueueSlotLastSent(id: number, time: number) {
    this.db
      .prepare("UPDATE queue_slots SET last_sent_at = ? WHERE id = ?")
      .run(time, id);
  }
