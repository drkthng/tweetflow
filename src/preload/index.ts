...
if (process.contextIsolated) {
  try {
    ...
    contextBridge.exposeInMainWorld("api", {
      scheduleTweet: (tweet: any) =>
        ipcRenderer.invoke("schedule-tweet", tweet),
      ...
      updateTweet: (id: number, tweet: any) =>
        ipcRenderer.invoke("update-tweet", id, tweet),
      deleteTweet: (id: number) => ipcRenderer.invoke("delete-tweet", id),
      softDeleteTweet: (id: number) =>
        ipcRenderer.invoke("soft-delete-tweet", id),
      softDeleteHistory: (id: number) =>
      ...
      dbInsertRow: (table: string, data: any) =>
        ipcRenderer.invoke("db-insert-row", table, data),
      dbUpdateRow: (table: string, id: number, data: any) =>
        ipcRenderer.invoke("db-update-row", table, id, data),
      dbDeleteRow: (table: string, id: number) =>
        ipcRenderer.invoke("db-delete-row", table, id),
      dbGetSchema: (table: string) =>
        ipcRenderer.invoke("db-get-schema", table),
    });
  } catch (error) {
    console.error(error);
  }
