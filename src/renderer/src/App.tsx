...
const App: React.FC = () => {
  const [blocks, setBlocks] = useState<
    { content: string; mediaPath: string | null }[]
  >([{ content: "", mediaPath: null }]);
  ...
  const [activeTab, setActiveTab] = useState<
    "queue" | "drafts" | "history" | "accounts" | "slots" | "database"
  >("queue");
  const [customSeparator, setCustomSeparator] = useState("||");

  // New Composer States
  const [tweetMode, setTweetMode] = useState<"scheduled" | "queued">(
    "scheduled",
  );
  ...
  const handleAction = async (isDraft: boolean) => {
    ...
    if (blocks.some((b, i) => b.content.length > getMaxChars(i))) {
      alert(
        "One or more tweets exceed the character limit (including numbering).",
      );
      return;
    }
  ...
  const handleDeleteAccount = async (id: number) => {
    if (
      !confirm(
        "Are you sure? This will NOT delete tweets associated with this account but they will fail to post.",
      )
    )
      return;
  ...
  const handleUpdateTweetAccount = async (
    tweetId: number,
    accountId: number,
  ) => {
  ...
  const handleEdit = (tweet: Tweet) => {
    ...
    setRecurrenceMonths(
      Math.round(tweet.recurrence_interval / (30 * 24 * 60 * 60 * 1000)),
    );
    setActiveIndex(0);
  };
            ...
            <div className="composer-stage">
              <TweetBlock
                ...
                maxChars={
                  280 -
                  (blocks.length > 1
                    ? `\n${activeIndex + 1}/${blocks.length}`.length
                    : 0)
                }
                suffix={
                  blocks.length > 1
            ...
            <div className="actions" style={{ marginTop: 0 }}>
              ...
              >
                ...
                <input
                  ...
                  value={threadDelay}
                  onChange={(e) =>
                    setThreadDelay(parseInt(e.target.value) || 1)
                  }
                  style={{
                    width: "60px",
                ...
                <div style={{ display: "flex", gap: "1rem" }}>
                  ...
                  >
                    <input
                      ...
                      name="mode"
                      value="scheduled"
                      checked={tweetMode === "scheduled"}
                      onChange={() => setTweetMode("scheduled")}
                    />
                    Scheduled
                  </label>
                  <label
                  ...
                  >
                    <input
                      ...
                      name="mode"
                      value="queued"
                      checked={tweetMode === "queued"}
                      onChange={() => setTweetMode("queued")}
                    />
                    Queued
                  </label>
                </div>
          ...
          <div className="card" style={{ marginTop: "1rem" }}>
            ...
            >
              ...
              <span style={{ fontSize: "0.7rem", opacity: 0.6 }}>
                Splits text at every "{customSeparator}"
              </span>
            </div>
            <textarea
              ...
              {[
                ...
                >
                  ...
                  {t !== "accounts" &&
                    t !== "slots" &&
                    t !== "database" &&
                    ` (${t === "queue" ? tweets.length : t === "drafts" ? drafts.length : history.length})`}
                </div>
              ))}
            </div>
            ...
            <div className="tab-content">
              {activeTab === "database" ? (
                ...
                <div style={{ padding: "1rem" }}>
                  ...
                  >
                    <input
                      placeholder="Account name (e.g. English)"
                    ...
                    <input
                      ...
                      value={newAccount.access_secret}
                      onChange={(e) =>
                        setNewAccount({
                          ...newAccount,
                    ...
                    >
                      <button onClick={handleAddAccount} style={{ flex: 1 }}>
                      ...
                      >
                        <div>
                          ...
                          <div style={{ fontSize: "0.7rem", opacity: 0.5 }}>
                            Last trigger:{" "}
                            {slot.last_sent_at
                              ? new Date(slot.last_sent_at).toLocaleString()
                              : "Never"}
                          </div>
                        </div>
                        <button
                ...
                (activeTab === "queue"
                  ...
                  <div key={tweet.id} className="queue-item">
                    ...
                    >
                      ...
                      >
                        <span
                          className={`status status-${tweet.status.toLowerCase()}`}
                        >
                          {tweet.status}
                        </span>
                        {activeTab === "queue" && (
                    ...
                    <div className="tweet-meta">
                      {activeTab === "queue"
                        ...
                        : activeTab === "drafts"
                          ? "Saved as draft"
                          : `Executed at: ${new Date(tweet.sent_at).toLocaleString()}`}
                    </div>
                  </div>
                ))
              )}
